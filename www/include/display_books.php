...
$query = "SELECT DISTINCT(isbn), title, price, ";
$query .= "image_url FROM all_book_data ";
...
{       
    ...
    case 2: 
        if ($_SESSION['author_action'] == 1 
			&& !isset($_SESSION['id_number']))
			$endline = " WHERE ";
        else
            $endline = " AND ";

        $endline .= sprintf(" title REGEXP \"^[%s-%s]\" ",
...
case 2:
    ...
    {   
    case 2:
        $endline .= sprintf(" AND title REGEXP \"^[%s-%s]\" ", 
			$_SESSION['tstart'], $_SESSION['tend']);
        break;           
    case 3:
        $endline .= sprintf(" AND title LIKE \"%s%%\" ",
			$_SESSION['talphachar']);
        break;
    default:
        break;
...
if ($result = mysqli_query($this->con, $query))
{
    ...
    while ($row = mysqli_fetch_assoc($result))
    {
        if (!array_key_exists($row['isbn'], $this->items))
        {
            ...
            Print "\" alt=\"book_image\"/>\n";
            Print "<h2 class=\"title\">";
            Print htmlspecialchars($row['title']) . "</h2>\n";
            Print "<p>Author:" . $this->get_authors($row['isbn']);
            Print "</p>\n";
            Print "<p class=\"isbn\">ISBN: " .$row['isbn'] . "</p>\n";
            Print "<p class=\"price\">$" .$row['price'] / 100 . "</p>\n";
            $this->createForm("",
                array(array('type' => 'hidden', 'name' => 'cart_action',
                 'value' => 1),
                array('type' => 'hidden', 'name' => 'isbn',
                 'value' => htmlspecialchars($row['isbn'])),
                array('type' => 'hidden', 'name' => 'qty',
                 'value' => 1),
                array('type' => 'submit', 'name' => 'add_to_cart',
                 'value' => 'Add to Cart')));
